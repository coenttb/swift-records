...
extension SnapshotIntegrationTests.Execution.Select {
    ...
    func selectWithWhere() async {
        await assertQuery(
            Reminder.where { $0.isCompleted }.select { ($0.id, $0.title, $0.isCompleted) }.order(by: \.id),
            sql: {
                """
    ...
    func selectWithNullChecks() async {
        await assertQuery(
            Reminder.where { $0.assignedUserID == nil }.select { ($0.id, $0.title, $0.assignedUserID) }.order(by: \.id),
            sql: {
                """
    ...
    func selectWithIn() async {
        ...
        await assertQuery(
            Reminder.where { $0.priority.in(priorities) }.select { ($0.id, $0.title, $0.priority) }.order(by: \.id),
            sql: {
                """
